package javarush.tasks.JavaSyntaxPro.tasks08.task1705;
    ...
    public static void runWorkingProcess() {
        for (Astronaut astronaut : astronauts) {
            if (astronaut instanceof Human) {
                Solution.pilot((Human) astronaut);
            }
            else if (astronaut instanceof Dog) {
                Solution.createDirection((Dog) astronaut);
            }
            else if (astronaut instanceof Cat) {
                Solution.research((Cat) astronaut);
            }
        }  
    }
