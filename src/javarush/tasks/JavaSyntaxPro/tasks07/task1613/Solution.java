package javarush.tasks.JavaSyntaxPro.tasks07.task1613;
...
public class Solution {
    ...
    public static boolean isWeekend(LocalDateTime dateTime) {
       int dayOfWeekNumber = dateTime.getDayOfWeek().getValue();
        int hoursOfDay = dateTime.getHour();

        return switch (dayOfWeekNumber) {
            case FRIDAY -> WEEKEND_START_FRIDAY_CUT_OFF_HOUR <= hoursOfDay;
            case SATURDAY -> true;
            case SUNDAY -> hoursOfDay < WEEKEND_END_SUNDAY_CUT_OFF_HOUR;
            default -> false;
        };
    }
}
